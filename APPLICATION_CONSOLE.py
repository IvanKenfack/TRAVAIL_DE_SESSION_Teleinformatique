...
def ProcessusPoigneDeMain(socket):
    ...
    # Extraction des informations du segment
    commande, numero_seq, numero_ack, drapeau, fenetrage1, tailleMorçeau1, checksum, nom_fichier, donnee = struct.unpack(
    ...
    if GenerateurSignatureHash(donnee) == checksum.rstrip(b"\x00"):
        ...
        # Creation et envoi du segment ACK / Finalisation  de la connexion
        segment = CreationSegment(b"", numero_seq, numero_ack, b"ACK", fenetrage, tailleMorçeau, signature_ACK, b"",
                                  b"ACK")
        EnvoiMessage(socket, segment, address_serveur)
        print("ACK envoyé")
        print()
...
while True:
    ...
    # Si la commande est "ls" ou "2", on affiche la liste des fichiers disponibles
        ...
        numero_seq = numero_ack
        # Creation du segment
        segment = CreationSegment(commande, numero_seq, numero_ack, b"", fenetrage, tailleMorçeau, b"", nom_fichier,
                                  b"")
